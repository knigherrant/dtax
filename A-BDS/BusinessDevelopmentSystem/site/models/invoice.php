 ...
 */
class BusinessSystemModelInvoice extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Invoice', $prefix = 'BusinessSystemTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_businesssystem.invoice', 'invoice', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_businesssystem.edit.invoice.data', array());
	...
	public function save($data){
        ...
        if($data['storage_type'] == 'file'){
            if(is_file(JPATH_SITE.'/'.  jSont::getDirPath('files').'/'.$data['storage_path_file'])){
                ...
                JFactory::getApplication()->enqueueMessage(JText::_('COM_BUSINESSSYSTEM_MSG_FILE_NOT_FOUND'), 'error');
                return false;
            }
        }else{
            $data['ext'] = '';
