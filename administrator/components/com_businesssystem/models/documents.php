 ...
 */
class BusinessSystemModelDocuments extends JModelList {
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__businesssystem_documents` AS a');
       $query->select('CONCAT(ac.firstname," ",ac.midname," ",ac.lastname) as name');
        $query->join('LEFT', '`#__businesssystem_accounts` AS ac ON ac.id = a.owner');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('LOWER(a.title) LIKE ' . $search . ' OR LOWER(a.title) LIKE ' . $search);
            }
        }
